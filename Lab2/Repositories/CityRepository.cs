    ...
    public class CityRepository
    {
        ...
        public double GetCityArea()
        {
            return _context.BlocksXml
                    .Root.Elements()
                    .Aggregate(0d, (sum, b) => sum + Parser.ToDouble(b.Element("Area")?.Value));
        }

        public double GetAreaByOnePerson()
        {
            if(GetCityArea() == 0)  return 0;

            var result = _context.BlocksXml
                         .Root.Elements()
                         .Sum(b => Parser.ToInt(b.Element("InhabitantsNumber")?.Value));

            return result / GetCityArea();
        }

        public Dictionary<ProjectType, IEnumerable<House>> GetHousesByTypes()
        {
            var query = from h in _context.HousesXml.Root.Elements()
                        group h by Parser.ToProjectType(h.Element("ProjectType")?.Value);
        ...
        {
            return _context.HousesXml
                           .Root.Elements()
                           .GroupBy(h => Parser.ToInt(h.Element("FloatsNumber")?.Value))
        ...
        public IEnumerable<House> GetNewHouses()
        {
            return _context.HousesXml
                   .Root.Elements()
                   .OrderByDescending(h => Parser.ToDateTimeOffset(h.Element("CreationDate")?.Value))
                   .Where(h => Parser.ToDateTimeOffset(h.Element("CreationDate")?.Value).Year > 2015)
                   .Select(h => h.ToHouse());
        }
        ...
        public IEnumerable<House> GetHappyHouses()
        {
            return _context.HousesXml
                           .Root.Elements()
                           .Where(h => h.Element("Code") != null && 
                                       h.Element("Code").Value.Contains("7"))
                           .Select(h => h.ToHouse());
        }
        ...
        public IEnumerable<ProjectType> GetSpecificTypes(int floats)
        {
            var result = from h in _context.HousesXml.Root.Elements()
                         where Parser.ToInt(h.Element("FloatsNumber")?.Value) >= floats
                         select Parser.ToProjectType(h.Element("ProjectType")?.Value);
        ...
        {
            var house = _context.HouseToBlocksXml
                        .Root.Elements()
                        .FirstOrDefault(hb => hb.Element("HouseCode")?.Value == houseCode);

            if (house is null) return $"No house with {houseCode} code.";

            var block = _context.BlocksXml
                        .Root.Elements()
                        .FirstOrDefault(b => b.Element("Code")?.Value == 
                                             house.Element("BlockCode")?.Value);

            return block is null ? 
                        "Block is unknown." : 
                        block.Element("AdministrationAddress")?.Value;
        }

        public IEnumerable<House> GetTheBiggestBlockInfo()
        {
            if (!_context.BlocksXml.Root.Elements().Any()) 
            ...
            var blockCodeAndHouses = from h in _context.HousesXml.Root.Elements()
                                     join hb in _context.HouseToBlocksXml.Root.Elements()
                                             on h.Element("Code")?.Value 
                                             equals hb.Element("HouseCode")?.Value
            ...
            var biggestBlock = (from b in _context.BlocksXml.Root.Elements()
                                orderby Parser.ToInt(b.Element("InhabitantsNumber")?.Value) 
                                descending
                                select b)
                               .FirstOrDefault();
            ...
            var result = blockCodeAndHouses
                        .FirstOrDefault(m => m.BlockCode == 
                                             biggestBlock.Element("Code")?.Value
                                       );
        ...
        {
            var result = from h in _context.HousesXml.Root.Elements()
                         orderby Parser.ToDateTimeOffset(h.Element("CreationDate")?.Value).Year
                         select h;
        ...
        public int GetPercentOfHighRiseByBlock(string blockCode)
        {
            ...
            var housesIsHighRise = from h in result.Houses
                                   where Parser.ToProjectType(h.Element("ProjectType")?.Value) ==
                                         ProjectType.HighRise
                                   select h;

            if(blockCodeAndHouses.Count() == 0)
                return 0;
        ...
        public IEnumerable<Block> GetBlocksWithMultiEntrencesHouses()
        {
            var result = from b in _context.BlocksXml.Root.Elements()
                         join hb in _context.HouseToBlocksXml.Root.Elements() 
                         on b.Element("Code")?.Value 
                         equals hb.Element("BlockCode")?.Value
                         where (from h in _context.HousesXml.Root.Elements()
                                where Parser.ToInt(h.Element("EntrencesNumber")?.Value) > 1
        ...
        public Block PlayInBlocks()
        {
            ...
            return _context.BlocksXml
                           .Root.Elements()
                           .FirstOrDefault(b => b.Element("Code")?.Value == block.BlockCode)
                           .ToBlock();
        }
